...
x_min=-np.pi
x_max= np.pi
X = np.linspace(x_min,x_max,1000001)
...
for hidden_layers in list_of_hl:
    ...
    if TRAINING:
        ...
        try:
            ...
            # Final Plot
            if WITHPLOT:
                y_dnn=regressor.predict(x={'X': X_pred}, as_iterable=False)
                plt.plot(X_pred, y_pred, label='function to predict')
                plt.plot(X_pred, y_dnn , label='DNNRegressor prediction')
                fig,(ax1,ax2)=plt.subplots(2,1,sharex=True)
                plt.sca(ax1)
                plt.plot(X_pred, y_pred, label='function to predict')
                plt.plot(X_pred, regressor.predict(x={'X': X_pred}, as_iterable=False),
                ...
                plt.plot(X_pred, y_dnn-y_pred , label='Delta')
