...
parser.add_argument('--max-time-to-evolve'                , type =float           , default=0.1     , help ='What is the maximum time the network should revert?')
parser.add_argument('--min-time-to-evolve'                , type =float           , default=0.001 , help ='What is the minimum time the network should revert?')
parser.add_argument('--steps-per-epoch'                   , type =int             , default=128     , help ='Steps per epoch')
...
parser.add_argument('--num-times-error-versus-time'       , type =int             , default=20      , help ='Times where the error should be evaluated')
parser.add_argument('--num-samples-for-error-versus-time' , type =int             , default=32      , help ='Samples  number for the error-versus-time plot')
parser.add_argument('--dropout'                           , type =float           , default=0.      , help ='Dropout (probably always stick to 0)')
parser.add_argument('--do-not-validation'                 , action ='store_true'  ,                   help ='Should the model be validated?')
parser.add_argument('--do-not-training'                   , action ='store_true'  ,                   help ='Should the model be trained?')
parser.add_argument('--do-not-error-vs-time'              , action ='store_true'  ,                   help ='Should the error versus time plot be computed?')
...
parser.add_argument('--plot-all-for-err-vs-time'          , action ='store_true'  ,                   help ='Plot full error versus time information (many-many plots to come)')
...
# Set options
TRAINING                      = not OPTIONS ['do_not_training']
VALIDATION                    = not OPTIONS ['do_not_validation']
ERROR_VERSUS_TIME_MEASUREMENT = not OPTIONS ['do_not_error_vs_time']
...
NUM_TIMES_ERR_VS_TIME         =     OPTIONS ['num_times_error_versus_time']
NUM_SAMPLES_FOR_ERR_VS_TIME   =     OPTIONS ['num_samples_for_error_versus_time']
PLOT_ALL                      =     OPTIONS ['plot_all_for_err_vs_time']
...
def get_XY_fixed_t(NUM_SAMPLES_PER_FILE, fixed_t=None):
    if fixed_t==None:
        ...
        if LEARN_INVERSE:
            return t,y,X
        return t,X,y
def generate_error_data(name, NUM_SAMPLES, NUM_TIMES_ERR_VS_TIME, OVERWRITE=False):
    filelist=glob(DATA_ROOT_PATH+DATA_PREFIX+str(name)+'*'+DATA_SUFFIX)
    if len(filelist)==0 or OVERWRITE:
        logging.info('Generate new '+str(name)+' data')
        t,X,y=get_XY_fixed_t(NUM_SAMPLES, NUM_TIMES_ERR_VS_TIME)
        if LEARN_INVERSE:
            y,X=X,y
...
generate_error_data(ERROR_VS_TIME_PREFIX,NUM_SAMPLES_FOR_ERR_VS_TIME,NUM_TIMES_ERR_VS_TIME,OVERWRITE=OVERWRITE)
...
while True:
    ...
    if VALIDATION:
        plt.ioff()
        logging.info('Do the validation')
        ...
        for i,y in enumerate(y_dnn):
            ...
            if LEARN_INVERSE:
                ...
                plt.plot(SAMPLING_SPACE , apply_heat_kernel( y_pred_for_plot, T[i] ) , label='Fwd Reconst')
                ...
                plt.plot(SAMPLING_SPACE , apply_heat_kernel( y_pred_for_plot, T[i] )-X[i] , label=r'$\Delta(t=$'+str(T[i])+'$)$')
    ...
    if ERROR_VERSUS_TIME_MEASUREMENT:
        plt.ioff()
        logging.info('\tStart error vs. time for '+model_name(parm))
        VALIDATION_PATH=DATA_ROOT_PATH+'/validation/'+model_name(parm)+'/'
        ...
        L2_t1=dict()
        for i,y in enumerate(y_dnn):
            if i >= len(X) or i >=NUM_TIMES_ERR_VS_TIME*NUM_SAMPLES_FOR_ERR_VS_TIME:
                break
            if LEARN_INVERSE:
                y_pred_for_plot=np.array(y['predictions'])
                tim = T[i][0]
        ...
        L2_t0_mean =np.array( [np.mean(L2_t0[t]) for t in Ts])
        L2_t0_min  =np.array( [min(L2_t0[t])     for t in Ts])
        L2_t0_max  =np.array( [max(L2_t0[t])     for t in Ts])
        L2_t0_err  =np.array( [np.std(L2_t0[t])  for t in Ts])
        L2_t1_mean =np.array( [np.mean(L2_t1[t]) for t in Ts])
        L2_t1_min  =np.array( [min(L2_t1[t])     for t in Ts])
        L2_t1_max  =np.array( [max(L2_t1[t])     for t in Ts])
        L2_t1_err  =np.array( [np.std(L2_t1[t])  for t in Ts])
        eps=6e-5
        plt.sca(ax1)
        plt.errorbar(Ts, L2_t0_mean, yerr=L2_t0_err, label='Statistics')
        plt.plot(Ts-eps,L2_t0_min, label='min')
        plt.plot(Ts+eps,L2_t0_max, label='max')
        plt.title('Reconstructed')
        plt.legend(loc='best')

        plt.sca(ax2)
        plt.errorbar(Ts, L2_t1_mean, yerr=L2_t1_err, label='Statistics')
        plt.plot(Ts-eps,L2_t1_min, label='min')
        plt.plot(Ts+eps,L2_t1_max, label='max')
        plt.title('Forwarded')
        plt.legend(loc='best')

        os.makedirs(VALIDATION_PATH, exist_ok=True)
        print(VALIDATION_PATH+'error_versus_time.png')
        plt.savefig(VALIDATION_PATH+'error_versus_time.png')
        plt.savefig(VALIDATION_PATH+'error_versus_time.pdf')
        plt.close(fig)
        logging.info('\tEnd error vs. time for '+model_name(parm))
